...
def extract_constituent_ordering(sentence):
    def check_dependents(head_id):
        ordering = {"O": None, "V": head_id}
        for dep in sentence:
                if dep["head"] == head_id:
                    if dep["deprel"] in ["obj"] and dep["upos"] in ["NOUN"]:
                        ordering["O"] = dep["id"]
...
for conllu_sent in conllu_sents:
    ...
    if curr_ordering_list:
        conllu_sent.metadata["83A"] = ";".join(curr_ordering_list)
        for curr_ordering in curr_ordering_list:
            ordering_counter[curr_ordering] += 1
    else:
        conllu_sent.metadata["83A"] = "_"

with open(f"{filepath}.83A.intermediate", "w") as f_out:
